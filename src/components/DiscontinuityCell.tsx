...
import { discontinuityCount } from "../format";
...
export function DiscontinuityCell({ summary, kind }: {
  ...
  const criterion = kind === "clicks"
    ? "Isolated impulse candidates up to 0.5 ms, with abrupt edges well above the surrounding signal changes. They can sound like clicks."
  ...
  return <td className={`${summary.count > 0 ? "c-warn" : "c-muted"} has-tip`} title={title}>{discontinuityCount(summary)}</td>;
}
